 ...
 */
class FWaterGlobalFlowCS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FWaterGlobalFlowCS);
	SHADER_USE_PARAMETER_STRUCT(FWaterGlobalFlowCS, FGlobalShader);

	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(uint32, GridSize)
		SHADER_PARAMETER(float, DeltaTime)
		SHADER_PARAMETER(float, FlowNoiseIntensity)
