...
✅ 调整Substeps（NS需要更多）  
✅ 添加WaterInteractionComponent到角色  
✅ 测试性能并调优
...
void AMyGameMode::BeginPlay()
{
    ...
    UWaterSubsystem* Water = GetWorld()->GetSubsystem<UWaterSubsystem>();
    
    // 根据需求选择配置
    #if PLATFORM_MOBILE
        Water->FluidConfig.SolverType = EFluidSolverType::ShallowWater;
        Water->FluidConfig.GridSize = 128;
    #else
        Water->FluidConfig.SolverType = EFluidSolverType::NavierStokes;
        Water->FluidConfig.GridSize = 256;
    #endif
    
    Water->FluidConfig.Viscosity = 0.02f;
    Water->FluidConfig.Damping = 0.05f;
    Water->bEnableSimulation = true;
}
```
